...
#define Radio_FMScanSen ((byte)((Radio_ScanLevel+8)*2))//DX
#define Radio_FMScanSen_Loc ((byte)((Radio_ScanLevel+TUNER_LEVEL_20dB+8)*2))//   Local Att=DX+20
...
#define StereoSeparation_Level_H  ((byte)((TUNER_LEVEL_20dB+8)*2))		//Stereo indication by LEVEL
#define StereoSeparation_Level_L  ((byte)((TUNER_LEVEL_14dB+8)*2))		//Stereo indication by LEVEL
...
typedef struct{         //area radio parameter 
	unsigned int FM_MaxFreq;			 //fm max freq
	unsigned int FM_MinFreq;                        //fm min freq
	unsigned int AM_MaxFreq;                       // am max freq
	unsigned int AM_MinFreq;                        //am min freq
	unsigned int FM_AutoSeekStep;               // fm step
	unsigned int FM_ManualSeekStep;           
	unsigned int AM_AutoSeekStep;              //am step
	unsigned int AM_ManualSeekStep;
}Radio_AreaConfigDef;
...
typedef struct 
{
    int Freq[MaxStationNum];   //0-current work freq,or backup freq;
}StationMemType;


/*band function*/
extern void Radio_SetBand(byte Band);
extern void Radio_NextBand(void);
extern byte Radio_IsFMBand(void);
extern byte Radio_GetCurrentBand(void);

/*preset function*/
extern void Radio_SaveCurrentFreq2Preset(byte Station);
extern void Radio_SelectPreset(byte Station);
extern void Radio_ClearCurrentStation(void);
extern byte Radio_GetCurrentStation(void);

/*radio freqency function*/
extern void Radio_SetFreq(byte mode, byte Band,int Freq);
extern unsigned int Radio_GetFreqStep(byte band);
extern void Radio_ChangeFreqOneStep(byte UpDown);
extern int Radio_GetCurrentFreq(void);
...
extern byte Radio_CheckStationStatus(void);
...
void Radio_Save_Station(byte StationNumber ,int StationFreq);
void Radio_StoreStationRecord(byte Band);
void Radio_SetSeekSenLevel(byte Lev);
void Radio_StoreStation(byte Band, byte Station);
...
int Radio_Get_RDS_Data(unsigned int*rds_data);
